...
// Copyright (c) 2014 The Primio developers
...
uint256 hashGenesisBlock("0000000641e7af2ee82de4860c94fde5583eb23f5e293a403915fe60486a1539");
...
const string strMessageMagic = "Primio Signed Message:\n";
                              ...
                              enum GetMinFee_mode mode) const
{
    ...
    if (fAllowFree)
    {
        if (nBlockSize == 1)
        {
            ...
            // (about 1500 Primio if made of 50 Primio inputs)
            if (nBytes < 1000)
                nMinFee = 0;
        }
        else
...
{
	// Initial reward per block is 50 coins
	int64 nSubsidy = 50 * COIN;
	// Rewards is cut in half every 100,000 blocks which is approximately 1 year
	nSubsidy >>= (nHeight/100000);
	//No more reward after block 600,000
	if(nHeight > 600000){nSubsidy = 0 * COIN;}
	return nSubsidy + nFees;
}

// Blocktime of 5 minutes and a difficulty retarget time of 1 hour.
...
unsigned int GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
	...
	// Commented out for debug overflowing reasons.
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0000000641e7af2ee82de4860c94fde5583eb23f5e293a403915fe60486a1539");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis block
        const char* pszTimestamp = "Why are you even looking here?";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1399051872;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 44576929;
        ...
        assert(block.hashMerkleRoot == uint256("0x47b83a36c417032a8bcb917984fb4e6afdde6eeb971a42424e521616d2db4493"));
        ...
        BOOST_FOREACH(CAddress& addr, vAddr)
        {
            ...
            if (addr.nTime > nSince && !pfrom->fGetAddr && vAddr.size() <= 10 && addr.IsRoutable())
            {
                // Relay to a limited number of other nodes
                {
                    ...
                    hashRand = HashJH(BEGIN(hashRand), END(hashRand));
                    ...
                    BOOST_FOREACH(CNode* pnode, vNodes)
                    {
                        ...
                        hashKey = HashJH(BEGIN(hashKey), END(hashKey));
...
bool ProcessMessages(CNode* pfrom)
{
    ...
    while (!pfrom->fDisconnect && it != pfrom->vRecvMsg.end()) {
        ...
        uint256 hash = HashJH(vRecv.begin(), vRecv.begin() + nMessageSize);
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("PrimioMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("PrimioMiner : ProcessBlock, block not accepted");
    }
...
{
    printf("PrimioMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("Primio-miner");
    ...
    try { while(true) {
        ...
        printf("Running PrimioMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
