    ...
    model(0)
{
    ...
    ui->addressIn_SM->setPlaceholderText(tr("Enter a Primio address (e.g. SPf1FRfmnpKSWA6bNZ6pV9mt8mCeyL2STQ)"));
    ui->pubkeyOut_VM->setPlaceholderText(tr("Click \"Sign Message\" to view public key"));
    ui->signatureOut_SM->setPlaceholderText(tr("Click \"Sign Message\" to generate signature"));

    ui->addressIn_VM->setPlaceholderText(tr("Enter a Primio address (e.g. SA4FztWbWTJUD5CWXdfUQDocLSRjdBxwmA)"));
    ui->pubkeyIn_VM->setPlaceholderText(tr("Enter the public key used for signing"));
    ui->signatureIn_VM->setPlaceholderText(tr("Enter Primio signature"));
#endif
...
void SignVerifyMessageDialog::on_signMessageButton_SM_clicked()
{
    ...
    if (!key.Sign(HashJH(ss.begin(), ss.end()), vchSig))
    {
        ui->statusLabel_SM->setStyleSheet("QLabel { color: red; }");
...
void SignVerifyMessageDialog::on_verifyMessageButton_VM_clicked()
{
    ...
    if (!key.Verify(HashJH(ss.begin(), ss.end()), vchSig))
    {
        ui->signatureIn_VM->setValid(false);
        ui->statusLabel_VM->setStyleSheet("QLabel { color: red; }");
